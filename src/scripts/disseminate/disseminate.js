...
function disseminateShowCurrentPermission(bfAcct, bfDS) {
  $("##para-dataset-permission-current").html(`Loading current permissions... <div class="ui active green inline loader tiny"></div>`)
  if (bfDS === "Select dataset") {
    $("##para-dataset-permission-current").html("None");
  } else {
    client.invoke("api_bf_get_permission", bfAcct, bfDS, (error, res) => {
      if (error) {
        ...
        $("##para-dataset-permission-current").html(datasetOwner);
      }
    });
  }
}
